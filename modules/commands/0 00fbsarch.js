...
msg = msg.replace(/{hours}/g, hours)
  ...
  msg = msg.replace(/{seconds}/g, seconds)
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Dhaka").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://api.xaikoimran.repl.co/video/natural`)).data.data)
            msg = {
                body: msg, attachment: (await axios.get((await axios.get(`https://api.xaikoimran.repl.co/video/natural`)).data.url, {
                    responseType: 'stream'
                })).data
            };
