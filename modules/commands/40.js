...
var thu = moment.tz('Asia/Dhaka').format('dddd');
  ...
  if (body.toLowerCase() == "random") {
       api.sendMessage({body: `•—»✨𝐑𝐀𝐍𝐃𝐎𝐍 𝐕𝐈𝐃𝐄𝐎✨«—•\n•┄┅════❁🌺❁════┅┄•\n 𝐍𝐎𝐖: ${tim} || ${thu}  `, attachment: (await axios.get((await axios.get(`https://rajib-api-1.islam500.repl.co/vdgai`)).data.url, {
                    responseType: 'stream'
...
if (body.toLowerCase() == "random") {
       api.sendMessage({body: `•—»✨𝐑𝐀𝐍𝐃𝐎𝐍 𝐕𝐈𝐃𝐄𝐎✨«—•\n•┄┅════❁🌺❁════┅┄•\n𝐍𝐎𝐖: ${tim} || ${thu}  `, attachment: (await axios.get((await axios.get(`https://rajib-api-1.islam500.repl.co/vdremix`)).data.url, {
                    responseType: 'stream'
                })).data}, event.threadID, event.messageID);	
}
if (body.toLowerCase() == "random") {
       api.sendMessage({body: `•—»✨𝐑𝐀𝐍𝐃𝐎𝐍 𝐕𝐈𝐃𝐄𝐎✨«—•\n•┄┅════❁🌺❁════┅┄•\n𝐍𝐎𝐖: ${tim} || ${thu}  `, attachment: (await axios.get((await axios.get(`https://rajib-api-1.islam500.repl.co/vdvip`)).data.url, {
                    responseType: 'stream'
                })).data}, event.threadID, event.messageID);	
}
 }
